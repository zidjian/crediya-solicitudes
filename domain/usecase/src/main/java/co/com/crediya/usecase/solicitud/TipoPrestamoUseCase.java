...
public class TipoPrestamoUseCase {
  ...
  public Mono<TipoPrestamo> findById(Long idTipoPrestamo) {
    if (idTipoPrestamo == null) {
      return Mono.error(
          new IllegalArgumentException("El id del tipo de préstamo no puede ser nulo"));
    }
    ...
    return tipoPrestamoRepository
        ...
        .switchIfEmpty(
            Mono.error(
                new NotFoundException("No existe un tipo de préstamo con id: " + idTipoPrestamo)))
        .onErrorMap(
            throwable -> {
              ...
              return new RuntimeException(
                  "Error al buscar el tipo de préstamo con id: " + idTipoPrestamo, throwable);
            });
  }
}
