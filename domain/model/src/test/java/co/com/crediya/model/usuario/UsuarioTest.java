...
class UsuarioTest {
  ...
  void debeCrearUsuarioConCamposMínimos() {
    ...
    // Act
    Usuario usuario = Usuario.builder().nombre(nombre).email(email).build();
  ...
  void debePermitirModificarCamposUsandoSetters() {
    // Arrange
    Usuario usuario =
        Usuario.builder().nombre("Pedro").email("pedro@example.com").activo(false).build();
  ...
  void debeCrearUsuarioInactivoPorDefecto() {
    // Arrange & Act
    Usuario usuario = Usuario.builder().nombre("Lucia").email("lucia@example.com").build();
