...
class SolicitudTest {
  ...
  void toSolicitud_ConDatosValidos_DebeCrearSolicitudExitosamente() {
    ...
    // Act
    Solicitud solicitud =
        Solicitud.toSolicitud(idUser, email, monto, plazo, idTipoPrestamo, idEstado);
  ...
  void constructor_CuandoIdUserEsNuloOVacio_DebeLanzarIllegalArgumentException(
      String idUserInvalido) {
    // Arrange
    String email = "test@example.com";
    BigDecimal monto = BigDecimal.valueOf(10000);
    LocalDate plazo = LocalDate.now().plusYears(1);
        ...
        assertThrows(
            ...
            () ->
                Solicitud.toSolicitud(
                    idUserInvalido, email, monto, plazo, idTipoPrestamo, idEstado));
  ...
  void constructor_CuandoEmailEsNuloOVacio_DebeLanzarIllegalArgumentException(
    ...
    IllegalArgumentException exception =
        assertThrows(
            ...
            () ->
                Solicitud.toSolicitud(
                    idUser, emailInvalido, monto, plazo, idTipoPrestamo, idEstado));
  ...
  @DisplayName(
      "constructor - Debe lanzar IllegalArgumentException cuando monto es menor o igual a cero")
  void constructor_CuandoMontoEsMenorOIgualACero_DebeLanzarIllegalArgumentException(
      String montoStr) {
    // Arrange
    String idUser = "12345678";
    String email = "test@example.com";
    ...
    IllegalArgumentException exception =
        assertThrows(
            ...
            () ->
                Solicitud.toSolicitud(
                    idUser, email, montoInvalido, plazo, idTipoPrestamo, idEstado));
  ...
  void constructor_ConIdEstadoNulo_DebeCrearSolicitudExitosamente() {
    ...
    // Act
    Solicitud solicitud =
        Solicitud.toSolicitud(idUser, email, monto, plazo, idTipoPrestamo, idEstadoNulo);
  ...
  void constructor_ConEspaciosEnIdUserYEmail_DebeTrimerarValores() {
    ...
    Solicitud solicitud =
        Solicitud.toSolicitud(
            idUserConEspacios, emailConEspacios, monto, plazo, idTipoPrestamo, idEstado);
  ...
  void cambiarEstado_ConNuevoEstado_DebeCrearNuevaInstanciaConEstadoCambiado() {
    ...
    Solicitud solicitudOriginal =
        Solicitud.toSolicitud(idUser, email, monto, plazo, idTipoPrestamo, idEstadoInicial);
  ...
  void fromDatabase_ConDatosValidos_DebeCrearSolicitudExitosamente() {
    ...
    // Act
    Solicitud solicitud =
        Solicitud.fromDatabase(idSolicitud, idUser, email, monto, plazo, idTipoPrestamo, idEstado);
  ...
  @Test
  @DisplayName(
      "calcularDeudaTotalMensual - Debe calcular correctamente con tasa de interés positiva")
  ...
  void calcularDeudaTotalMensual_ConTasaInteresCero_DebeCalcularCorrectamente() {
    ...
    Solicitud solicitud =
        Solicitud.toSolicitud(
            "123", "test@test.com", BigDecimal.valueOf(12000), fechaPlazo, 1L, 1L);
    ...
    if (meses <= 0) meses = 1;
    BigDecimal expected =
        BigDecimal.valueOf(12000).divide(BigDecimal.valueOf(meses), 2, RoundingMode.HALF_UP);
  ...
  void calcularDeudaTotalMensual_ConPlazoPasado_DebeUsarUnMes() {
    ...
    Solicitud solicitud =
        Solicitud.toSolicitud(
            "123", "test@test.com", BigDecimal.valueOf(12000), fechaPasada, 1L, 1L);
  ...
  void calcularDeudaTotalMensual_ConPlazoMuyCercano_DebeUsarUnMes() {
    ...
    Solicitud solicitud =
        Solicitud.toSolicitud(
            "123", "test@test.com", BigDecimal.valueOf(12000), fechaCercana, 1L, 1L);
    BigDecimal tasaInteresAnual = BigDecimal.valueOf(12.0);
    ...
    assertNotNull(resultado);
    assertTrue(resultado.compareTo(BigDecimal.ZERO) > 0);
    // For very close dates (0 months), it should use 1 month, so result should be close to the full
    // amount
    assertTrue(resultado.compareTo(BigDecimal.valueOf(10000)) >= 0);
  }
