...
public class UsuarioHttpClient implements UsuarioRepository, UsuarioValidacionRepository {
    ...
    private static final String USUARIOS_BY_DOCUMENTO_ENDPOINT = "/api/v1/usuarios/documento/{documento}";
    ...
    public Mono<Usuario> obtenerUsuarioPorId(Long idUsuario) {
        ...
        return ReactiveSecurityContextHolder.getContext()
                ...
                .flatMap(token -> webClient
                        ...
                        .doOnNext(usuario -> log.debug("Usuario encontrado - ID: {}, Documento: {}",
                                 usuario.getIdUsuario(), usuario.getDocumentoIdentidad()))
                        .map(this::mapToUsuario)
                        .doOnSuccess(usuario -> log.info("Usuario con ID {} obtenido exitosamente", idUsuario))
                        .onErrorResume(WebClientResponseException.NotFound.class, ex -> {
                            log.warn("Usuario con ID {} no encontrado", idUsuario);
                            return Mono.empty();
                        })
                        .onErrorResume(WebClientResponseException.class, ex -> {
                            log.error("Error al obtener usuario con ID {}: {}", idUsuario, ex.getMessage());
                            return Mono.empty();
                        })
                        .onErrorResume(Exception.class, ex -> {
                            log.error("Error genérico al obtener usuario con ID {}: {}", idUsuario, ex.getMessage());
                            return Mono.empty();
                        }));
    }
        ...
        return ReactiveSecurityContextHolder.getContext()
                ...
                .flatMap(token -> webClient
                        ...
                        .doOnNext(usuario -> log.debug("Usuario encontrado - ID: {}, Documento: {}",
                                 usuario.getIdUsuario(), usuario.getDocumentoIdentidad()))
                        .map(this::mapToUsuario)
                        .doOnSuccess(usuario -> log.info("Usuario con documento {} obtenido exitosamente", documentoIdentidad))
                        .onErrorResume(WebClientResponseException.NotFound.class, ex -> {
                            log.warn("Usuario con documento {} no encontrado", documentoIdentidad);
                            return Mono.empty();
                        })
                        .onErrorResume(WebClientResponseException.class, ex -> {
                            log.error("Error al obtener usuario con documento {}: {}", documentoIdentidad, ex.getMessage());
                            return Mono.empty();
                        })
                        .onErrorResume(Exception.class, ex -> {
                            log.error("Error genérico al obtener usuario con documento {}: {}", documentoIdentidad, ex.getMessage());
                            return Mono.empty();
                        }));
    }
