...
public class TipoPrestamoRepositoryAdapter implements TipoPrestamoRepository {
  ...
  public Mono<TipoPrestamo> findById(Long idTipoPrestamo) {
    ...
    return repository
        ...
        .doOnSuccess(
            tipoPrestamo ->
                log.debug("[TIPO_PRESTAMO_ADAPTER] Tipo de préstamo encontrado: {}", tipoPrestamo))
        .doOnError(
            error ->
                log.error(
                    "[TIPO_PRESTAMO_ADAPTER] Error al buscar tipo de préstamo por ID {}: {}",
                    idTipoPrestamo,
                    error.getMessage()));
  }
  ...
  public Mono<TipoPrestamo> findByNombre(String nombre) {
    ...
    return repository
        ...
        .doOnSuccess(
            tipoPrestamo ->
                log.debug(
                    "[TIPO_PRESTAMO_ADAPTER] Tipo de préstamo encontrado por nombre: {}",
                    tipoPrestamo))
        .doOnError(
            error ->
                log.error(
                    "[TIPO_PRESTAMO_ADAPTER] Error al buscar tipo de préstamo por nombre {}: {}",
                    nombre,
                    error.getMessage()));
  }
  ...
  public Mono<Boolean> existeByNombre(String nombre) {
    log.debug(
        "[TIPO_PRESTAMO_ADAPTER] Verificando existencia de tipo de préstamo por nombre: {}",
        nombre);
    return repository
        .findByNombre(nombre)
        .hasElement()
        .doOnSuccess(
            existe ->
                log.debug("[TIPO_PRESTAMO_ADAPTER] Tipo de préstamo {} existe: {}", nombre, existe))
        .doOnError(
            error ->
                log.error(
                    "[TIPO_PRESTAMO_ADAPTER] Error al verificar existencia de tipo de préstamo {}: {}",
                    nombre,
                    error.getMessage()));
  }
  ...
  public Mono<Boolean> existeById(Long idTipoPrestamo) {
    log.debug(
        "[TIPO_PRESTAMO_ADAPTER] Verificando existencia de tipo de préstamo por ID: {}",
        idTipoPrestamo);
    return repository
        .existsById(idTipoPrestamo)
        .doOnSuccess(
            existe ->
                log.debug(
                    "[TIPO_PRESTAMO_ADAPTER] Tipo de préstamo con ID {} existe: {}",
                    idTipoPrestamo,
                    existe))
        .doOnError(
            error ->
                log.error(
                    "[TIPO_PRESTAMO_ADAPTER] Error al verificar existencia de tipo de préstamo con ID {}: {}",
                    idTipoPrestamo,
                    error.getMessage()));
  }
}
