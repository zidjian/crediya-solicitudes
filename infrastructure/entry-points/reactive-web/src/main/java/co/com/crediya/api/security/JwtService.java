...
public class JwtService {
  ...
  public JwtService(
      @Value("${jwt.secret:mySecretKey123456789012345678901234567890}")
          String secretKeyString, // Obtiene los env o uso los datos por defecto
      @Value("${jwt.expiration:24}")
          long expirationTimeInHours // Obtiene los env o uso los datos por defecto
      ) {
    ...
    if (secretKeyString == null
        ...
        || secretKeyString.getBytes().length < 64) {
      ...
      if (keyBytes.length < 64) {
        log.warn(
            "JWT secret key muy corta ({} bytes). Generando clave segura para HS512...",
            keyBytes.length);
        this.secretKey = Jwts.SIG.HS512.key().build();
      } else {
  ...
  public String generateToken(
      Long userId, String email, String nombre, String apellido, String rolNombre, String idUser) {
